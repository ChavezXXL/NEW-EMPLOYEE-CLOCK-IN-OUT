...
function setupEventListeners() {
  const elements = {
    ...
    prevWeekButton: { id: 'prevWeekButton', event: 'click', handler: () => changeWeek(-7) },
    nextWeekButton: { id: 'nextWeekButton', event: 'click', handler: () => changeWeek(7) }
  };
...
// Function to generate and download pay day sheet
...
async function generateEmployeePayDayContent(employee, startDate, endDate, settings) {
  ...
  const timesheet = timesheetSnapshot.val() || {};
  const relevantEntries = Object.values(timesheet).filter(entry => 
    new Date(entry.timestamp) >= startDate && new Date(entry.timestamp) <= endDate
  );

  const { totalWorkHours, regularHours, overtimeHours, totalPay } = calculateTotalHours(relevantEntries, employee.payRate, settings);

  return `
    <h3>${employee.name} (${employee.id})</h3>
    <p>Period: ${startDate.toDateString()} - ${endDate.toDateString()}</p>
    <table>
      <tr><th>Total Hours</th><td>${totalWorkHours.toFixed(2)}</td></tr>
      <tr><th>Regular Hours</th><td>${regularHours.toFixed(2)}</td></tr>
      <tr><th>Overtime Hours</th><td>${overtimeHours.toFixed(2)}</td></tr>
      <tr><th>Total Pay</th><td>$${totalPay.toFixed(2)}</td></tr>
    </table>
  `;
}

function calculateTotalHours(entries, payRate, settings) {
  let totalWorkHours = 0;
  let regularHours = 0;
  let overtimeHours = 0;

  const overtimeThreshold = settings.overtimeThreshold || 8; // Default to 8 if not set
  const overtimeMultiplier = settings.overtimeMultiplier || 1.5; // Default to 1.5 if not set

  entries.forEach((entry, index) => {
    if (entry.type === 'clockout') {
      const clockIn = entries.find(e => e.type === 'clockin' && e.timestamp < entry.timestamp);
      if (clockIn) {
        const hoursWorked = (entry.timestamp - clockIn.timestamp) / 3600000; // Convert ms to hours
        totalWorkHours += hoursWorked;

        if (totalWorkHours <= overtimeThreshold) {
          regularHours += hoursWorked;
        } else {
          const remainingRegularHours = Math.max(0, overtimeThreshold - (totalWorkHours - hoursWorked));
  ...
  return { totalWorkHours, regularHours, overtimeHours, totalPay };
}

// Add these functions to view and edit employees
